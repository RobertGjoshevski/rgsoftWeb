...
export default function DomeGallery({
    ...
    const openItemFromElement = useCallback(
        el => {
            ...
            // Function to calculate size based on image aspect ratio
            const calculateImageSize = (naturalWidth, naturalHeight, maxWidth, maxHeight) => {
                ...
                const maxW = typeof maxWidth === 'string' ? parseFloat(maxWidth) : maxWidth;
            ...
            const handleImageResize = () => {
                ...
                function processImageResize() {
                    const naturalWidth = img.naturalWidth || frameR.width;
                    const naturalHeight = img.naturalHeight || frameR.height;

                    // Parse openedImageWidth and openedImageHeight as max dimensions
                    const maxWidth = openedImageWidth ? (typeof openedImageWidth === 'string' ? parseFloat(openedImageWidth) : openedImageWidth) : frameR.width;
                    const maxHeight = openedImageHeight ? (typeof openedImageHeight === 'string' ? parseFloat(openedImageHeight) : openedImageHeight) : frameR.height;

                    // Calculate size preserving aspect ratio
                    const { width: displayWidth, height: displayHeight } = calculateImageSize(naturalWidth, naturalHeight, maxWidth, maxHeight);
                    ...
                    // Resize to calculated dimensions after initial transform
                    const onFirstEnd = ev => {
